...
const Blog = ({ headerFooter, posts }) => {
  ...
  const tags = [
    ...new Set(posts.flatMap((item) => item.tags?.map(tag => tag.name.toLowerCase()) || [])),
  ];
  ...
  const updateURL = (selectedCategories, selectedTags) => {
    ...
    history.pushState(null, "", `/archive${url.searchParams.toString() ? `?${url.searchParams.toString()}` : ""}`);
  };
  ...
  const filteredPosts = posts.filter((item) => {
    const matchesCategory = selectedCategories.length === 0 || selectedCategories.includes(item.categories[0].name.toLowerCase());
    const matchesTag = selectedTags.length === 0 || item.tags?.some(tag => selectedTags.includes(tag.name.toLowerCase()));
