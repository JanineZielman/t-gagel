		...
		<Layout headerFooter={ headerFooter || {} } seo={ postData?.yoast_head_json ?? {} }>
			...
			<div className={`post ${categories[0].slug}`}>
				<div className="post-wrapper">
					...
					<h2>{new Date(postData?.date).toLocaleDateString("nl-nl")}</h2>
...
export async function getStaticProps( { params } ) {
	...
	const categoriesRes = await axios.get(`https://cms.gagel.nl/wp-json/wp/v2/categories?include[]=${categoryIds.join(',')}`);
